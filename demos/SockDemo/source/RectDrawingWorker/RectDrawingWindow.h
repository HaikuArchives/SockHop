// This file, and all other source files that make up
// SockDemo, are placed into the public domain.  - Jeremy Friesner
 
#ifndef _RectDrawingWindow_H_
#define _RectDrawingWindow_H_

// This is the window that actually does the line drawing!
// One object of this class is held by each RectDrawingWindow.

#include <interface/Window.h>
#include <app/Messenger.h>
#include <interface/Point.h>

#define LDW_COMMAND_CLOSING 'ldwc'

class RectDrawingWindow : public BWindow
{
public:
    RectDrawingWindow(BRect rect, const char * name, BMessenger reportClosingTo);
    virtual ~RectDrawingWindow();
    
    virtual void MessageReceived(BMessage * msg);
    
    void SetTempo(int32 newTempo);
    
private:
    static long DrawThreadStub(void * data);
    long DrawThread();
    BPoint RandomPointIn(const BRect & frame) const;
    
    BView * _drawArea;
    BMessenger _reportCloseTo;
    thread_id _drawThread;
    int32 _tempo;
};

#endif
