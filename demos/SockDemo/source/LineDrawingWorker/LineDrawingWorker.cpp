   ...
   : SHWorker(archive), _name(NULL), _window(NULL)
{
   ...
   {
      // Pick a random name...
      sprintf(temp2, "LineDrawingWorker %Li",system_time());
      temp = temp2;      
   }
...
{
   if (!validate_instantiation(archive, "LineDrawingWorker")) return NULL;
   return new LineDrawingWorker(archive);
}
...
GetAddOnSpec(SHFileSpec & spec) const
{
    ...
    if ((ret = spec.AddFlavor(SHFlavor("add-ons/x86/LineDrawingWorker", SH_ARCH_BEOS_X86, true))) != B_NO_ERROR) 
    {
       printf("LineDrawingWorker::GetAddOnSpec:  Error adding intel LineDrawingWorker flavor 'add-ons/x86/LineDrawingWorker'\n");
       return ret;
    }
    if ((ret = spec.AddFlavor(SHFlavor("add-ons/ppc/LineDrawingWorker", SH_ARCH_BEOS_PPC, true))) != B_NO_ERROR) 
    {
       printf("LineDrawingWorker::GetAddOnSpec:  Error adding powerpc LineDrawingWorker flavor 'add-ons/ppc/LineDrawingWorker'\n");
       return ret;
    }
    return B_NO_ERROR;
}
   ...
   switch(msg->what)
   {
      case LDW_COMMAND_STARTUP:
         if (_window == NULL)
         {
            ...
            _window = new LineDrawingWindow(frame, _name, GetMessenger());
            _window->Show();
         }
      break;
      ...
      case LDW_COMMAND_CLOSING:
         if (GetMessenger().SendMessage(B_QUIT_REQUESTED) != B_NO_ERROR)
            printf("LineDrawingWorker: Warning, couldn't send the 'I'm closing' BMessage!\n");
      break;
      ...
      default:
         printf("LineDrawingWorker [%s]:  Unknown BMessage received, listing follows:\n", GetName());
         msg->PrintToStream();
      break;
   }
}
